"use strict";
// src/scenarios/products/get-products-sort-asc.ts
...
// === Custom Metrics ===
const sortAscDuration = new metrics_1.Trend('sort_asc_duration');
// === Load Parameters ===
...
function default_1() {
    const url = `${BASE_URL}/products?sortBy=title&order=asc`;
    (0, k6_1.group)('Sort Products - Ascending', () => {
        const start = Date.now();
        const res = http_1.default.get(url);
        const duration = Date.now() - start;
        sortAscDuration.add(duration);
        const body = res.json();
        (0, k6_1.check)(res, {
