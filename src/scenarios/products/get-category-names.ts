...
// === Custom Metrics ===
const categoryNamesDuration = new Trend('category_names_duration');
...
export default function () {
  ...
  group('List Category - Names', () => {
    ...
    const body = res.json() as string[] | null;

    check(res, {
      'status is 200': (r) => r.status === 200,
      'response is an array of strings': () =>
        (Array.isArray(body) && body?.every((c) => typeof c === 'string')) ?? false,
    });
  });
}
