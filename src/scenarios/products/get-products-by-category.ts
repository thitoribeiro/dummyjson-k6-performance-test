...
// === Custom Metrics ===
const productsByCategoryDuration = new Trend('products_by_category_duration');
...
export default function () {
  ...
  group('Search Products - By Category', () => {
    ...
    check(res, {
      ...
      'category is smartphones': () =>
        body?.products?.every((p: Product) => p.category === 'smartphones') ?? false,
      'total is 16': () => body?.total === 16,
    });
  });
}
