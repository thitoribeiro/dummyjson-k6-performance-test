...
// === Custom Metrics ===
const selectFieldsDuration = new Trend('select_fields_duration');
...
export default function () {
  ...
  group('Select Fields - title,price', () => {
    ...
    check(res, {
      ...
      'products have id, title, price': () =>
        body?.products?.every((p: Product) => p.id && p.title && p.price) ?? false,
    });
  });
}
